...
final class BeeminderAPI {
    ...
    func getDatapoints(for slug: String, sort: String?, count: Int?, page: Int?, per: Int?) -> AnyPublisher<[Datapoint], Error> {
        var url = baseURL
    ...
    func createDatapoint(slug: String, value: Double, timestamp: Int?, daystamp: String?, comment: String?, requestid: String?) -> AnyPublisher<Datapoint, Error> {
        struct Body: Codable {
            let value: Double
            let timestamp: Int?
            let daystamp: String?
